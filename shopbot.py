...
def batch_add_products(products_data):
    ...
    for product in products_data:
        ...
        if product["category"] not in ["money", "weapon", "item", "car", "fashion", "rentcar"]:
            errors.append(f"หมวดหมู่ '{product['category']}' ไม่ถูกต้องสำหรับสินค้า '{product['name']}'")
            continue
...
class LegacyConfirmButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            qr_embed.set_image(url=load_qrcode_url())
            qr_embed.set_footer(text="กรุณาสแกน QR Code เพื่อชำระเงิน โอนเงินและแคปหลักฐานส่งให้แอดมิน")
...
async def admin_examples_command(ctx):
    ...
    embed = create_admin_examples_embed()
    await ctx.send(embed=embed)
...
async def help_command(ctx):
    ...
    embed.add_field(
        ...
        value="ใช้คำสั่ง `!ตัวอย่างแอดมิน` เพื่อดูรายละเอียดและตัวอย่างการใช้คำสั่งสำหรับแอดมินทั้งหมด",
        inline=False
    )
